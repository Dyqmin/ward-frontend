 ...
 */
export function snapshotThenStream(
  bus: MessageBus,
  bed: BedId,
): Observable<VitalsFrame[]> {
  const snapshot$ = bus
    ...
    scan((acc, e) => {
      switch (e.kind) {
        case 'snapshot': {
          ...
          return [...e.frames, ...acc.filter((f) => f.ts > last)].slice(
            -VITALS_WINDOW,
          ); // keep frames that arrived early
        }
        case 'frame':
          return acc.at(-1)?.ts === e.frame.ts
            ? acc
            : [...acc, e.frame].slice(-VITALS_WINDOW);
        default:
