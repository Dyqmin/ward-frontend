...
const NAMES = [
  'Bea Placebo', 'Cal Ibration', 'Dee Hydration', 'Earl E. Bird', 'Faye Kename', 'Gus Tatory', "Hal O'Gen",
  'Ivy Drip', 'Jo King', 'Kay Pillary', 'Lou Kocyte', 'Mo Nitor', 'Nell Ebulizer', 'Otto Scope', 'Pip Ette',
];
...
export function mockFrame(bed: BedId, tick: number, hrOverride?: number): VitalsFrame {
  const h = hash(bed);
  let hr = Math.round(68 + (h % 16) + 4 * Math.sin(tick / 7 + h) + 2 * Math.sin(tick / 2.3));
  // ICU-3 crosses the demo threshold for 15 s every 90 s
  if (bed === 'ICU-3' && tick % 90 >= 75) hr = 131 + ((tick % 90) - 75);
  if (hrOverride !== undefined) hr = Math.round(hrOverride + Math.sin(tick / 1.7));
  const spo2 = Math.min(100, Math.round(96 + (h % 3) + Math.sin(tick / 11 + h)));
...
function outOfRangeCodes(f: VitalsFrame): AlarmCode[] {
  ...
  if (THRESHOLDS.hr.high !== undefined && f.hr > THRESHOLDS.hr.high) codes.push('hr.high');
  if (THRESHOLDS.hr.low !== undefined && f.hr < THRESHOLDS.hr.low) codes.push('hr.low');
  if (THRESHOLDS.spo2.low !== undefined && f.spo2 < THRESHOLDS.spo2.low) codes.push('spo2.low');
  if (THRESHOLDS.rr.high !== undefined && f.rr > THRESHOLDS.rr.high) codes.push('rr.high');
  if (THRESHOLDS.rr.low !== undefined && f.rr < THRESHOLDS.rr.low) codes.push('rr.low');
  return codes;
}

const valueOf = (f: VitalsFrame, code: AlarmCode): number => f[code.split('.')[0] as 'hr' | 'spo2' | 'rr'];

const accepted = <T>(value: T): CommandResult<T> => ({ status: 'accepted', value });
const forbidden = (reason: string): { status: 'forbidden'; reason: string } => ({ status: 'forbidden', reason });
const isNurse = (actor: ParticipantId | null): actor is NurseId => !!actor?.startsWith('nurse_');
const isDoctor = (actor: ParticipantId | null): actor is DoctorId => !!actor?.startsWith('dr_');

export class MockWard {
  readonly patients = new Map<BedId, Patient>();
  readonly orders = new Map<MedOrderId, MedOrder & { given?: { by: NurseId; at: string } }>();
  ...
  constructor() {
    ...
    for (const bed of ALL_BEDS) {
      ...
      this.patients.set(bed, {
        ...
        bed,
        admittedAt: new Date(Date.UTC(2026, 8, 18 + (i % 6), 8 + (i % 9))).toISOString(),
      });
      const drug = DRUGS[i % DRUGS.length];
      if (drug) this.addOrder({ patientId: id, ...drug }, SEED_DOCTOR, new Date(Date.UTC(2026, 8, 24, 9, i)).toISOString());
      i++;
    }
  }
  ...
  addOrder(draft: Pick<MedOrder, 'patientId' | 'drug' | 'doseMg' | 'route'>, by: DoctorId, at: string): MedOrder {
    const order: MedOrder = { id: `med_${this.seq++}`, ...draft, status: 'ordered', orderedBy: by, createdAt: at };
    this.orders.set(order.id, order);
    return order;
  }

  activeAlarms(ward: Ward): AlarmEvent[] {
    return [...this.alarms.values()].filter((a) => wardOf(a.bed) === ward).map((a) => a.latest);
  }
  ...
  process(frame: VitalsFrame, ctx: MockContext): void {
    ...
    for (const code of out) {
      const existing = [...this.alarms.values()].some((a) => a.bed === frame.bed && a.code === code);
      if (existing) continue;
      const alarmId: AlarmId = `alarm_${frame.bed.toLowerCase()}_${code.replace('.', '_')}_${this.seq++}`;
      const event: AlarmEvent = { status: 'raised', alarmId, bed: frame.bed, code, value: valueOf(frame, code) };
      const rec: AlarmRecord = { alarmId, bed: frame.bed, code, latest: event, raisedAt: ctx.now, inRangeSince: null };
      this.alarms.set(alarmId, rec);
      ctx.emit(topic, event);
    }

    for (const rec of [...this.alarms.values()].filter((a) => a.bed === frame.bed)) {
      const inRange = !out.includes(rec.code);
      switch (rec.latest.status) {
        case 'raised':
          if (ctx.now - rec.raisedAt > ESCALATE_AFTER_MS) {
            broadcast({ status: 'escalated', alarmId: rec.alarmId, bed: rec.bed, to: 'doctor' }, rec);
          }
          break;
        case 'acknowledged':
          if (!inRange) rec.inRangeSince = null;
          else if (rec.inRangeSince === null) rec.inRangeSince = ctx.now;
          else if (ctx.now - rec.inRangeSince >= END_AFTER_IN_RANGE_MS) this.alarms.delete(rec.alarmId); // ends silently
          break;
        case 'snoozed':
          ...
          if (inRange) {
            ...
            broadcast({ status: 'raised', alarmId: rec.alarmId, bed: rec.bed, code: rec.code, value: valueOf(frame, rec.code) }, rec);
          }
          break;
        case 'escalated':
          break;
...
export function createWardFixtures(ward = new MockWard()) {
  type VitalsStreams = { [B in BedId as `/topic/vitals.${B}`]: ReturnType<MockWard['vitalsStream']> };
  ...
  return {
    ...
    replies: {
      ...
      '/app/vitals.history': ({ bed, minutes }) => {
        ...
        for (let t = now - minutes * 60; t < now; t++) frames.push(mockFrame(bed, t, ward.hrOverrides.get(bed)));
        return frames;
      },

      '/app/medication.list': ({ bed }) => {
        const patient = ward.patients.get(bed);
        return patient ? [...ward.orders.values()].filter((o) => o.patientId === patient.id) : [];
      },
      ...
      '/app/alarms.ack': ({ alarmId, performedAt }, ctx) => {
        ...
        if (rec.latest.status === 'acknowledged' || rec.latest.status === 'snoozed') {
          return { status: 'conflict', by: rec.latest.by, at: rec.latest.status === 'acknowledged' ? rec.latest.at : performedAt };
        }
        rec.latest = { status: 'acknowledged', alarmId, bed: rec.bed, by: ctx.actor, at: performedAt };
        ctx.emit(`/topic/alarms.${wardOf(rec.bed)}`, rec.latest);
        return accepted(null);
      },

      '/app/alarms.snooze': ({ alarmId, minutes, performedAt }, ctx) => {
        if (!isNurse(ctx.actor)) return forbidden('Only nurses can snooze alarms');
        const rec = ward.alarms.get(alarmId);
        if (!rec) return forbidden(`Alarm ${alarmId} is not active`);
        if (rec.latest.status === 'snoozed') return { status: 'conflict', by: rec.latest.by, at: performedAt };
        const until = new Date(Date.parse(performedAt) + minutes * 60_000).toISOString();
        rec.latest = { status: 'snoozed', alarmId, bed: rec.bed, by: ctx.actor, until };
        ctx.emit(`/topic/alarms.${wardOf(rec.bed)}`, rec.latest);
        return accepted({ until });
      },

      '/app/medication.order': ({ patientId, drug, doseMg, route, performedAt }, ctx) => {
        if (!isDoctor(ctx.actor)) return forbidden('Only doctors can order medication');
        if (!ward.patientById(patientId)) return forbidden(`Unknown patient ${patientId}`);
        if (!(doseMg > 0)) return forbidden('doseMg must be positive');
        const order = ward.addOrder({ patientId, drug, doseMg, route }, ctx.actor, performedAt);
        return accepted({ id: order.id });
      },

      '/app/medication.given': ({ id, performedAt }, ctx) => {
        if (!isNurse(ctx.actor)) return forbidden('Only nurses can give medication');
      ...
      '/app/monitor.set': ({ bed, hr }) => {
        if (!ward.patients.has(bed)) return forbidden(`Bed ${bed} is empty`);
        if (hr < 20 || hr > 220) return forbidden('hr must be between 20 and 220');
